<%
  ...
  def external_url(refname):
    ...
    """
    parts = refname.split('.')  # akr
    return '%s.m.html#%s' % (parts[1], refname) # akr
...
<%def name="show_module(module)">
  ...
  <section id="section-items">
    ...
    % for c in classes:
      ...
      <div class="class">
      <%
      # Prevent empty Parent sections
      nmro = []
      for m in mro:
       if  m.refname != "builtins.object" and m.refname != "builtins.Exception" and m.refname != "builtins.BaseException" and m.name != c.name:
         nmro.append(m)
      %>
        % if len(nmro) > 0:
